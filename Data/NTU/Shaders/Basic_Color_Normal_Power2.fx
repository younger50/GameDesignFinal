...
VS_OUTPUT2 GY_vs(float4 pos : POSITION,
		 float3 nor : NORMAL)
{
   VS_OUTPUT2 out1 = (VS_OUTPUT2) 0;
   float3 a = pos.xyz;
 
   a.xyz += nor*0.5f;

   out1.pos = mul(float4(a,1.0f), mWVP);
...
{   
   ...
   float3 H = normalize(normalize(vCam) + L);
   ...
   oCol.rgb=colorDiffuse*(ambLgt*1.5f + diff*mainLightColor*dif) + colorSpecular*(spec*spe*mainLightColor*0.7f); 
...
technique Basic_Color_Normal_Power
{
   ...
   pass P0
   {
     // ZWriteEnable=false;
      CullMode=cw;
      VertexShader = compile vs_2_0 GY_vs();
      PixelShader = compile ps_2_0 GY_ps();      
   }
