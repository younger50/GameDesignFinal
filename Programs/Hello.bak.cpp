...
public:
	...
	void loadPlayerActor(){
		// Get character actions
		idleID = actor.GetBodyAction(NULL, "Idle");
		runID  = actor.GetBodyAction(NULL, "Run");
		walkID = actor.GetBodyAction(NULL, "Walk");

		walk2RunID[0] = actor.CreateCrossBlendAction(NULL,"walkToRun","Walk","Run",10.0f);
		walk2RunID[1] = actor.CreateCrossBlendAction(NULL,"runToWalk","Run","Walk",10.0f);

		idle2WalkID[1] = actor.CreateCrossBlendAction(NULL,"walkToIdle","Walk","Idle",10.0f);
		idle2WalkID[0] = actor.CreateCrossBlendAction(NULL,"idleToWalk","Idle","Walk",10.0f);
		
		idle2RunID[0] = actor.CreateCrossBlendAction(NULL,"idleToRun","Idle","Run",10.0f);
		idle2RunID[1] = actor.CreateCrossBlendAction(NULL,"runToIdle","Run","Idle",10.0f);
	}
...
void FyMain(int argc, char **argv){
   // create a new world
   BOOL4 beOK = FyStartFlyWin32("Homewrok03", 50, 20, 1280, 800, FALSE);
